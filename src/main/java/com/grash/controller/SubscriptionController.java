...
public class SubscriptionController {
    ...
    @PostMapping("")
    @PreAuthorize("hasSubscription('ROLE_CLIENT')")
    @ApiResponses(value = {//
            @ApiResponse(code = 500, message = "Something went wrong"), //
            @ApiResponse(code = 403, message = "Access denied")})
    public Subscription create(@ApiParam("Subscription") @RequestBody Subscription subscriptionReq, HttpServletRequest req) {
        User user = userService.whoami(req);
    ...
    @ApiResponses(value = {//
            ...
            @ApiResponse(code = 403, message = "Access denied"), //
            @ApiResponse(code = 404, message = "Subscription not found")})
    public Subscription patch(@ApiParam("Subscription") @RequestBody SubscriptionPatchDTO subscription, @ApiParam("id") @PathVariable("id") Long id,
